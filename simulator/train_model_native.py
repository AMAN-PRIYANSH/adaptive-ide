"""
train_model_native.py — Train ML models on natively-complete real sessions
============================================================================
Trains two models:
...
Best model saved to backend/models/ability_model_native.pkl

This is the same pipeline as train_model_real.py, run on
real_sessions_native.jsonl — the 881-row / 85-session subset of
real_sessions.jsonl with the 156 reconstructed (filled-in) rows removed,
keeping only natively-complete rows. Same 80/20 session-split evaluation
plus 5-fold GroupKFold cross-validation grouped by session_id, so results
are directly comparable to the 1037-row run.
"""
...
DATA_FILE  = os.path.join(os.path.dirname(__file__), '..', 'backend', 'data', 'real_sessions_native.jsonl')
...
# ── Save best model ───────────────────────────────────────────────────
model_path = os.path.join(MODEL_DIR, "ability_model_native.pkl")
meta_path  = os.path.join(MODEL_DIR, "model_meta_native.json")
